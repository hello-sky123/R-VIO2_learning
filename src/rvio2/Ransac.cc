...
void Ransac::ComputeE(const int nIterNum, const Eigen::MatrixXf& Points1,
...
int Ransac::CountVotes(const Eigen::MatrixXf& Points1,
  ...
  for (const int& idx : vInlierCandidateIndexes) {
    if (mbUseSampson) {
      ...
      if (AlgebraicError(Points1.col(idx), Points2.col(idx), E) <
          mnAlgebraicThrd)
        nVotes++;
    }
  }
...
void Ransac::FindInliers(const Eigen::MatrixXf& Points1,
                         const Eigen::MatrixXf& Points2,
                         const Eigen::Matrix3f& R, int& nInliers,
  ...
  for (int i = 0; i < nInlierCandidates; ++i) {
    ...
    if (mbUseSampson) {
      ...
      float nError =
          AlgebraicError(Points1.col(idx), Points2.col(idx), WinnerE);
      if (nError > mnAlgebraicThrd || std::isinf(nError) ||
          std::isnan(nError)) {
        // Mark as outlier
        vInlierFlags.at(idx) = 0;
        nOutliers++;
