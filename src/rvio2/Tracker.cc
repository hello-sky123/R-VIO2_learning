...
void Tracker::DisplayTrack(const int nImageId, const cv::Mat& image,
...
void Tracker::DisplayNewer(const int nImageId, const cv::Mat& image,
...
void Tracker::VisualTracking(const int nImageId, const cv::Mat image,
  ...
  // LK method
  cv::calcOpticalFlowPyrLK(mLastImage, image, mvFeatPtsToTrack, vFeatPts,
                           vInlierFlags, vErrors, winSize, 3, termCriteria,
  ...
  if (std::accumulate(vInlierFlags.begin(), vInlierFlags.end(), 0) > 0) {
    ...
    mpRansac->FindInliers(PointsForRansac, MatchesForRansac, mRr, nInliers,
                          vInlierFlags);

    if (nInliers == 0) {
      std::cerr
          << "Visual Tracking: lost all features, refresh if the refill fails!"
          << "\n";
      mbRefreshVT = true;
    }
  } else {
  ...
  if (mbShowTrack) {
    ...
    DisplayTrack(nImageId, image, mvFeatPtsToTrack, vFeatPts, vInlierFlags,
                 imTrack);
  ...
  if (!mvFeatCandidates.empty()) {
    ...
    int nNewFeats = mpFeatureDetector->FindNewer(mvFeatCandidates,
                                                 mvFeatPtsToTrack, vNewFeatPts);
    ...
    if (nNewFeats > 0) {
      ...
      if (mbRefreshVT) mbRefreshVT = false;
    }
  }
...
bool Tracker::start(const int nImageId, const cv::Mat& image,
  ...
  if (nFeats == 0) {
    ...
    if (!mbRestartVT && !mbRefreshVT) mbRestartVT = true;

    return false;
  }

  if (mbRestartVT) mbRestartVT = false;
...
void Tracker::manage(const int nImageId, const cv::Mat& image,
                     const Eigen::Matrix3f& RcG, const Eigen::Vector3f& tcG,
  ...
  mpFeatureDetector->DetectWithSubPix(image, 1.5 * mnMaxFeatsPerImage, 1,
                                      mvFeatCandidates);
}
