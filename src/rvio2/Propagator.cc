...
void Propagator::CreateNewFactor(const std::vector<ImuData>& vImuData,
  ...
  for (const ImuData& data: vImuData) {
    ...
    if (wn < mnSmallAngle) {
      ...
      f2 = .5 *
           (pow(wn * dt, 2) - 2 * cos(wn * dt) - 2 * wn * dt * sin(wn * dt) +
            2) /
           pow(wn, 4);
...
void Propagator::LocalQR(const int nImageId,
  ...
  if (nImageId > mnLocalWindowSize) {
    LocalFactor.leftCols(L) =
        tempLocalSR.bottomRightCorner(L, L).triangularView<Eigen::Upper>();
  } else {
    LocalFactor.setZero(L + 6, L + 7);
    LocalFactor.leftCols(L + 6) = tempLocalSR.bottomRightCorner(L + 6, L + 6)
