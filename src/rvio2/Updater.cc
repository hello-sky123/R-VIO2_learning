...
Updater::Updater(const cv::FileStorage& fsSettings) {
  ...
  mFeatPub =
      mUpdaterNode.advertise<visualization_msgs::Marker>("/rvio2/landmarks", 1);
...
int Updater::triangulate(
    const int nTrackLength, const std::vector<cv::Point2f>& vRevFeatMeas,
    const std::vector<Eigen::Matrix<float, 7, 1>>& vRevRelCamPoses, float& phi,
    float& psi, float& rho) {
  phi = atan2(vRevFeatMeas.front().y, sqrt(pow(vRevFeatMeas.front().x, 2) + 1));
  psi = atan2(vRevFeatMeas.front().x, 1);
  rho = 0.;
  if (fabs(phi) > .5 * M_PI || fabs(psi) > .5 * M_PI) return -1;
  ...
  InitLM(phi, psi, rho, nTrackLength, vRevFeatMeas, vRevRelCamPoses, HTRinvH,
         HTRinve, cost);
  ...
  while (nIter < maxIter) {
    ...
    if (dpfinv.norm() < 1e-6) break;

    InitLM(new_phi, new_psi, new_rho, nTrackLength, vRevFeatMeas,
           vRevRelCamPoses, new_HTRinvH, new_HTRinve, new_cost);
    ...
    if (new_cost < cost) {
      ...
      if (fabs(cost - new_cost) < 1e-6 || fabs(dpfinv(2)) < 1e-6) break;
  ...
  if (fabs(phi) > .5 * M_PI || fabs(psi) > .5 * M_PI || std::isnan(rho) ||
      std::isinf(rho) || rho < 0)
    return -1;
...
void Updater::InitLM(
    const float phi, const float psi, const float rho, const int nTrackLength,
    const std::vector<cv::Point2f>& vRevFeatMeas,
    const std::vector<Eigen::Matrix<float, 7, 1>>& vRevRelCamPoses,
    Eigen::Matrix3f& HTRinvH, Eigen::Vector3f& HTRinve, float& cost) {
...
void Updater::GetRevRelPoses(
    const int type, const int nTrackLength, const Eigen::VectorXf& Winx,
  ...
  for (int i = 1; i < nTrackLength; ++i) {
    ...
    Eigen::Vector3f t =
        x2.tail(3) + QuatToRot(QuatInv(x2.head(4))) * x1.tail(3);

    vRevRelImuPoses.at(i) << RotToQuat(R), t;
    vRevRelCamPoses.at(i) << RotToQuat(mRci * R * mRic),
        mRci * R * mtic + mRci * t + mtci;
  }
}

bool Updater::CreateNewFactor(
    std::pair<int, Type>& pFeatInfo,
    const std::vector<cv::Point2f>& vRevFeatMeas,
    const std::vector<Eigen::Matrix<float, 7, 1>>& vRevRelImuPoses,
    const std::vector<Eigen::Matrix<float, 7, 1>>& vRevRelCamPoses,
    const std::deque<Eigen::Vector3f>& qRevLocalw,
    const std::deque<Eigen::Vector3f>& qRevLocalv, Eigen::MatrixXf& Hf,
    Eigen::MatrixXf& HP, Eigen::MatrixXf& HW, Eigen::VectorXf& r,
    Eigen::Vector3f& xf) {
  ...
  float phi, psi, rho;
  int rval =
      triangulate(nTrackLength, vRevFeatMeas, vRevRelCamPoses, phi, psi, rho);
  if (rval == -1) {
    if (type != POSE_ONLY) pFeatInfo.second = BAD;

    return false;
  } else if (rval == 0) {
    if (type != POSE_ONLY) pFeatInfo.second = UNUSED;
  ...
  for (int i = 0; i < nTrackLength; ++i) {
    ...
    if (i > 0) {
      ...
      for (int j = 0; j < i; ++j) {
        Eigen::Matrix3f R1T =
            QuatToRot(vRevRelImuPoses.at(j).head(4)).transpose();
        Eigen::Vector3f t1 = -R1T * vRevRelImuPoses.at(j).tail(3);
        Eigen::Matrix3f R2T =
            QuatToRot(vRevRelImuPoses.at(j + 1).head(4)).transpose();

        Eigen::Matrix3f Vx = SkewSymm(mRic * epfinv + rho * mtic - rho * t1);

        subHt +=
            HRR * (-Vx * R1T * qRevLocalw.at(j) + rho * R1T * qRevLocalv.at(j));

        tempHW.block(2 * i, 6 * (nTrackLength - 2 - j), 2, 6)
            << -HRR * Vx * R1T,
            rho * HRR * R2T;
      }

      subHqci = Hproj * ((SkewSymm(Rc * epfinv) - rho * SkewSymm(Rc * mtci)) *
                             (I - Rc) +
                         rho * SkewSymm(mRci * t));
...
void Updater::CreateNewFactor(const cv::Point2f& z, const Eigen::Vector3f& pfG,
                              const Eigen::Vector3f& pfG_fej,
                              const Eigen::Matrix<float, 7, 1>& xG,
                              const Eigen::Matrix<float, 7, 1>& xk,
                              const Eigen::Vector3f& wk,
                              const Eigen::Vector3f& vk, Eigen::MatrixXf& Hf,
                              Eigen::MatrixXf& HG, Eigen::MatrixXf& HP,
                              Eigen::MatrixXf& Hk, Eigen::VectorXf& r) {
  r.resize(2);
  Hf.resize(2, 3);
  HG.resize(2, 9);
  HP.resize(2, 7);
  ...
  Eigen::Matrix<float, 2, 3> Hproj;
  Hproj << 1. / h_fej(2), 0, -h_fej(0) / pow(h_fej(2), 2), 0, 1. / h_fej(2),
      -h_fej(1) / pow(h_fej(2), 2);
  ...
  HP << Hproj * SkewSymm(mRci * pfk_fej), Hproj,
      HR * SkewSymm(pfk_fej) * wk - HR * vk;
  Hk << HR * SkewSymm(pfk_fej), -HR * Rk, Eigen::MatrixXf::Zero(2, 9);
}

void Updater::LocalQR(const Eigen::MatrixXf& H, const Eigen::VectorXf& r,
...
void Updater::update(
    const int nImageId, const std::unordered_map<int, Feature*>& mFeatures,
  ...
  if (vFeatMeasForExploration.empty() && vFeatInfoForInitSlam.empty() &&
      vFeatInfoForPoseOnly.empty()) {
    mvLostActiveFeatureIDs.swap(vActiveFeatureIDs);

    composition(nImageId, mFeatures, vActiveFeatureIDs, nDimOfWinx, nDimOfWinSR,
                Localx, LocalFactor);
  ...
  LocalFactor.bottomRightCorner(9 + 7 + nDimOfWinSR, 9 + 7 + nDimOfWinSR + 1)
      .leftCols(9 + 7 + nDimOfWinSR)
      .triangularView<Eigen::Upper>()
      .solveInPlace(NavSRinv);
  ...
  if (!vFeatMeasForExploration.empty()) {
    ...
    for (std::vector<std::pair<int, cv::Point2f>>::const_iterator vitMeas =
             vFeatMeasForExploration.begin();
      ...
      // Get feature index
      auto vit =
          std::find_if(vActiveFeatureIDs.begin(), vActiveFeatureIDs.end(),
      ...
      Eigen::VectorXf tempr;
      CreateNewFactor((*vitMeas).second, pfG, pfG_fej, xG, xk, wk, vk, tempHf,
                      tempHG, tempHP, tempHk, tempr);
      ...
      H.conservativeResizeLike(Eigen::MatrixXf::Zero(
          nNewRows, 3 * nActiveFeatures + 9 + 7 + nDimOfWinSR));
    ...
    int n =
        std::accumulate(vFeatureStatuses.begin(), vFeatureStatuses.end(), 0);
  ...
  if (!vFeatInfoForInitSlam.empty()) {
    ...
    GetRevRelPoses(INIT_SLAM, mnLocalWindowSize + 1, Winx, vRevRelImuPoses,
                   vRevRelCamPoses);

    std::vector<std::pair<int, Type>>::iterator vitInfo =
        vFeatInfoForInitSlam.begin();
    std::vector<std::vector<cv::Point2f>>::const_iterator vitMeas =
        vvFeatMeasForInitSlam.begin();

    for (; vitInfo != vFeatInfoForInitSlam.end(); ++vitInfo, ++vitMeas) {
      int id = (*vitInfo).first;
      Feature* pFeature = mFeatures.at(id);

      std::vector<cv::Point2f> vRevFeatMeas((*vitMeas).rbegin(),
                                            (*vitMeas).rend());

      Eigen::MatrixXf tempHf, tempHP, tempHW;
      Eigen::VectorXf tempr;
      Eigen::Vector3f xf;
      if (!CreateNewFactor(*vitInfo, vRevFeatMeas, vRevRelImuPoses,
                           vRevRelCamPoses, qRevLocalw, qRevLocalv, tempHf,
                           tempHP, tempHW, tempr, xf))
        continue;
      ...
      if (type == INIT_SLAM) {
        ...
        tempHx.leftCols(7 + N) << tempHP.bottomRows(M - 3),
            tempHW.bottomRows(M - 3);
        temprx = tempr.tail(M - 3);

        float val =
            chi2(tempHx, temprx,
                 NavSRinv.bottomRightCorner(7 + nDimOfWinSR, 7 + nDimOfWinSR));
        ...
        nNewPoints++;
        Hf.conservativeResizeLike(Eigen::MatrixXf::Zero(
            3 * nNewPoints, 7 + nDimOfWinSR + 3 * nNewPoints));
    ...
    if (nNewPoints > 0) {
      ...
      tempM.bottomRightCorner(l3, l3 + 1) =
          LocalFactor.bottomRightCorner(l3, l3 + 1);
      tempM.block(l1, l1, l2, l2) = Hf.rightCols(l2);
      tempM.block(l1, l1 + l2 + 9, l2, 7 + nDimOfWinSR + 1)
          << Hf.leftCols(7 + nDimOfWinSR),
          rf;
  ...
  if (!vFeatInfoForPoseOnly.empty()) {
    ...
    std::vector<std::pair<int, Type>>::iterator vitInfo =
        vFeatInfoForPoseOnly.begin();
    std::vector<std::vector<cv::Point2f>>::const_iterator vitMeas =
        vvFeatMeasForPoseOnly.begin();
    ...
    for (; vitInfo != vFeatInfoForPoseOnly.end(); ++vitInfo, ++vitMeas) {
      ...
      std::vector<Eigen::Matrix<float, 7, 1>> vRevRelImuPoses, vRevRelCamPoses;
      GetRevRelPoses(type, nTrackLength, Winx, vRevRelImuPoses,
                     vRevRelCamPoses);
      ...
      if (val < CHI2_THRESHOLD[M - 1]) {
        ...
        if (type == POSE_ONLY_M) (*vitInfo).second = BAD;
  ...
  Localx.segment(nStateOffset, 4) =
      QuatMul(dqG, Localx.segment(nStateOffset, 4));
  ...
  Localx.segment(nStateOffset, 4) =
      QuatMul(dqP, Localx.segment(nStateOffset, 4));
  ...
  for (int i = 0; i < nWinSize; ++i) {
    ...
    Localx.segment(nStateOffset, 4) =
        QuatMul(dqw, Localx.segment(nStateOffset, 4));
    Localx.segment(nStateOffset + 4, 3) += dLocalx.segment(nErrorOffset + 3, 3);
...
void Updater::ComposeQR(const int nIdx, const int nDim,
                        Eigen::MatrixXf& LocalFactor) {
  int L = LocalFactor.rows();
...
void Updater::composition(const int nImageId,
                          ...
                          std::vector<int>& vActiveFeatureIDs,
                          const int nDimOfWinx, const int nDimOfWinSR,
  ...
  if (nNewActiveFeatures > 0) {
    ...
    for (int i = 0; i < nNewActiveFeatures; ++i) {
      ...
      epfinv << cos(phi) * sin(psi), sin(phi), cos(phi) * cos(psi);
      epfinv_fej << cos(phi_fej) * sin(psi_fej), sin(phi_fej),
          cos(phi_fej) * cos(psi_fej);

      Eigen::Vector3f pfG = RkGT * (1. / rho * mRic * epfinv + mtic - pkG);
      Eigen::Vector3f pfG_fej =
          RkGT * (1. / rho_fej * mRic * epfinv_fej + mtic - pkG);

      Eigen::Matrix<float, 3, 2> Jang;
      Jang << -sin(phi_fej) * sin(psi_fej), cos(phi_fej) * cos(psi_fej),
          cos(phi_fej), 0, -sin(phi_fej) * cos(psi_fej),
          -cos(phi_fej) * sin(psi_fej);

      Jf.block(3 * i, 3 * i, 3, 3) << 1. / rho_fej * RkGT * mRic * Jang,
          -1. / pow(rho_fej, 2) * RkGT * mRic * epfinv_fej;
      JG.block(3 * i, 0, 3, 6) << -SkewSymm(pfG_fej) * RkGT, -RkGT;
      JP.block(3 * i, 0, 3, 7)
          << -RkGT * mRic * SkewSymm(1. / rho_fej * epfinv_fej - mtci),
          -RkGT * mRic, Eigen::Vector3f::Zero();
  ...
  if (nLostActiveFeatures > 0) {
    int nDimOfSR =
        3 * (nActiveFeatures + nNewActiveFeatures) + 9 + 7 + nDimOfWinSR;
    Eigen::MatrixXf tempM =
