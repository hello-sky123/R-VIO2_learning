...
System::System(const std::string& strSettingsFile) {
  ...
  if (mbRecordOutputs) {
    ...
    fPoseResults.open(pkg_path + "/stamped_traj_estimate.txt",
                      std::ofstream::out);
  }
}
...
bool System::initialize(const ImageData& Image,
  ...
  if (mbRecordOutputs) {
    ...
    fPoseResults << std::setprecision(19) << im_last_timestamp << " " << pGk(0)
                 << " " << pGk(1) << " " << pGk(2) << " " << qkG(0) << " "
                 << qkG(1) << " " << qkG(2) << " " << qkG(3) << std::endl;
  }
...
void System::run() {
  ...
  mpTracker->track(nImageId, pMeasurements.first.Image, RcG, tcG, nMapPtsNeeded,
                   mmFeatures);

  // Save local velocities
  Eigen::Vector3f w =
      pMeasurements.second.back().AngularVel - Localx.tail(6).head(3);
  ...
  mpUpdater->update(
      nImageId, mmFeatures, mpTracker->mvFeatMeasForExploration,
  ...
  ROS_INFO("T_CI: %.6f %.6f %.6f %.6f %.6f %.6f %.6f", Localx(14), Localx(15),
           Localx(16), Localx(10), Localx(11), Localx(12), Localx(13));
  ROS_INFO("td: %.6f\n", mnCamTimeOffset);
