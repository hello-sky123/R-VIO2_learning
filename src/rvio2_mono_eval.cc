...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    ROS_ERROR(
        "Usage: roslaunch rvio2 launch_file[path_to_settings path_to_rosbag]");
    ros::shutdown();
    return -1;
  }
  ...
  for (const rosbag::MessageInstance& m: view) {
    ...
    if (imuMsg != NULL && (m.getTopic() == "/imu0" || m.getTopic() == "/imu")) {
      ...
      if ((int)imuMsg->header.seq != lastseq + 1 && lastseq != -1)
        ROS_ERROR("IMU message drop! curr seq: %d expected seq: %d.",
                  imuMsg->header.seq, lastseq + 1);
      lastseq = imuMsg->header.seq;

      Eigen::Vector3f angular_velocity(imuMsg->angular_velocity.x,
                                       imuMsg->angular_velocity.y,
                                       imuMsg->angular_velocity.z);
      Eigen::Vector3f linear_acceleration(imuMsg->linear_acceleration.x,
    ...
    if (imgMsg != NULL && (m.getTopic() == "/cam0/image_raw" ||
                           m.getTopic() == "/camera/image_raw")) {
      static int lastseq = -1;
      if ((int)imgMsg->header.seq != lastseq + 1 && lastseq != -1)
        ROS_ERROR("Image message drop! curr seq: %d expected seq: %d.",
                  imgMsg->header.seq, lastseq + 1);
