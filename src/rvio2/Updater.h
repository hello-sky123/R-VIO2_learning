...
class Updater {
 ...
 private:
  int triangulate(
      ...
      float& phi, float& psi, float& rho);

  void composition(const int nImageId,
                   const std::unordered_map<int, Feature*>& mFeatures,
                   std::vector<int>& vActiveFeatureIDs, const int nDimOfWinx,
                   const int nDimOfWinSR, Eigen::VectorXf& Localx,
                   Eigen::MatrixXf& LocalFactor);

  void ComposeQR(const int nIdx, const int nDim, Eigen::MatrixXf& LocalFactor);

  void LocalQR(const Eigen::MatrixXf& H, const Eigen::VectorXf& r,
               Eigen::MatrixXf& LocalFactor);

  void ReorderQR(const std::vector<int>& vFeatureStatuses,
                 std::vector<int>& vFeatureIDs, Eigen::MatrixXf& LocalFactor);

  void CreateNewFactor(const cv::Point2f& z, const Eigen::Vector3f& pfG,
                       const Eigen::Vector3f& pfG_fej,
                       const Eigen::Matrix<float, 7, 1>& xG,
                       const Eigen::Matrix<float, 7, 1>& xk,
                       const Eigen::Vector3f& wk, const Eigen::Vector3f& vk,
                       Eigen::MatrixXf& Hf, Eigen::MatrixXf& HG,
                       Eigen::MatrixXf& HP, Eigen::MatrixXf& Hk,
                       Eigen::VectorXf& r);

  bool CreateNewFactor(
  ...
  void InitLM(const float phi, const float psi, const float rho,
              const int nTrackLength,
              const std::vector<cv::Point2f>& vRevFeatMeas,
              const std::vector<Eigen::Matrix<float, 7, 1>>& vRevRelCamPoses,
              Eigen::Matrix3f& HTRinvH, Eigen::Vector3f& HTRinve, float& cost);

  inline float chi2(const Eigen::MatrixXf& H, const Eigen::VectorXf& r,
